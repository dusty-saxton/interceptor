...
// Hunt and tone-scan pacing, defined here (above INTERCEPTOR_TimeSlice10ms)
// because that function drives them. They count REAL 10ms ticks:
// INTERCEPTOR_Engine_Tick runs at the variable main-loop rate - thousands
// of times per second - so counting its calls made every one of these
// timeouts roughly 100x too short. Hunt reset itself long before the
// hardware frequency counter's 0.2s measurement could complete even once,
// and both tone scans exhausted their whole attempt budget in microseconds.
static bool     sHuntOwnsTuner     = true;
static uint16_t sHuntTurnTicks     = 0; // real 10ms ticks this hunt turn has run
static uint16_t sHuntCssPollDelay  = 0; // real 10ms ticks until hunt's next tone-scan poll
static uint16_t sSweepCssPollDelay = 0; // real 10ms ticks until sweep's next tone-scan poll
...
void INTERCEPTOR_TimeSlice10ms(void) {
    // Real-time pacing for hunt and the tone scans. These MUST be counted
    // here, not in INTERCEPTOR_Engine_Tick - that runs at the variable
    // main-loop rate, so counting its calls made every timeout roughly
    // 100x too short: hunt kept resetting itself before the hardware
    // frequency counter's 0.2s measurement could finish, and both tone
    // scans exhausted their attempt budget in microseconds.
    if (gSniffingEnabled && sHuntOwnsTuner) sHuntTurnTicks++;
    if (sHuntCssPollDelay  > 0) sHuntCssPollDelay--;
    if (sSweepCssPollDelay > 0) sSweepCssPollDelay--;
...
static void Do_BandSweep_Cycle(void) {
    ...
    if (sSweepCssActive) {
        // Same real-10ms pacing as hunt's tone scan - see HUNT_CSS_POLL_10MS.
        if (sSweepCssPollDelay > 0) return;
        sSweepCssPollDelay = HUNT_CSS_POLL_10MS;
...
void INTERCEPTOR_Engine_Tick(void) {
    ...
    if (gSniffingEnabled) {
        if (sHuntOwnsTuner) {
            ...
            Do_Hunt_Cycle();
            // sHuntTurnTicks is counted in INTERCEPTOR_TimeSlice10ms so it
            // measures real time, giving the frequency counter the ~0.6s
            // it genuinely needs for three stable reads.
            if (sHuntState == HUNT_IDLE && stateBefore != HUNT_IDLE) {
                sHuntOwnsTuner = false;
                sHuntTurnTicks = 0;
            } else if (sHuntTurnTicks >= HUNT_MAX_TURN_TICKS) {
                Hunt_Reset();
                sHuntOwnsTuner = false;
                sHuntTurnTicks = 0;
            }
